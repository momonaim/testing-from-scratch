...
- [x] Fichier créé (routes séparées de l'API `/api/tasks`)
    ...
    <table>
      ...
      <tbody>
        ...
        >
          ...
          <td th:text="${task.completed} ? '✅ Terminée' : '⏳ En cours'">
            Statut
          </td>
          <td>
            <a class="btn btn-edit" th:href="@{/tasks/{id}/edit(id=${task.id})}"
              >Modifier</a
            >
            <form
              class="inline"
...
- [x] Fichier créé
- [x] Chaque ligne a bien la classe `task-item` (nécessaire pour les futurs tests UI)
...
<html xmlns:th="http://www.thymeleaf.org">
  ...
  <body>
    <h1 th:text="${task.id} != null ? 'Modifier la tâche' : 'Nouvelle tâche'">
      Formulaire
    </h1>
...
- [x] Fichier créé
- [x] Les champs ont les `id` attendus (`title`, `description`) pour les futurs tests UI
...
- [x] `http://localhost:8080/` redirige vers `/tasks`
...
- [x] `http://localhost:8080/tasks` affiche la liste (vide au départ)
- [x] Cliquer sur "+ Nouvelle tâche" ouvre le formulaire
- [x] Créer une tâche → redirection vers la liste → la tâche apparaît
- [x] Cliquer "Modifier" → le formulaire est pré-rempli → modifier → sauvegarder → changement visible
- [x] Cliquer "Supprimer" → confirmation → la tâche disparaît de la liste

### 5.7 Commit Git

```bash
git add .
git commit -m "05: UI: feat: interface web Thymeleaf (liste, creation, edition, suppression)"
```
...
- [x] Les 4 opérations (liste, création, édition, suppression) fonctionnent depuis le navigateur
- [x] Les sélecteurs CSS/id utilisés (`.task-item`, `#title`, `#description`, `button[type='submit']`) sont stables et documentés pour les tests UI
- [x] Aucune erreur dans la console du navigateur ni dans les logs Spring Boot
