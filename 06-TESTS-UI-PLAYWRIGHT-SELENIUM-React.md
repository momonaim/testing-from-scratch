...
- [ ] Bien comprendre cette différence avant de continuer
...
- [ ] Dossier `e2e-tests/` créé à la racine du repo (à côté de `backend/` et `frontend/`)
...
- [ ] Fichier configuré (le backend, lui, doit être lancé séparément - voir A.3)
...
test.describe('Gestion des tâches', () => {
  ...
  test('création d\'une tâche', async ({ page }) => {
    ...
    await expect(lastTask.getByTestId('task-title')).toHaveText('Tâche Playwright');
  });
  ...
  test('modification d\'une tâche', async ({ page }) => {
    ...
    // Modifier
    await page.getByTestId('task-item').last().getByTestId('task-edit-button').click();
    await page.getByTestId('task-title-input').fill('Titre modifié');
    await page.getByTestId('task-submit-button').click();

    const lastTask = page.getByTestId('task-item').last();
    await expect(lastTask.getByTestId('task-title')).toHaveText('Titre modifié');
  });
  ...
  test('suppression d\'une tâche', async ({ page }) => {
    ...
    await page.getByTestId('task-item').last().getByTestId('task-delete-button').click();
...
```bash
git add .
git commit -m "test: automatisation E2E React avec Playwright et Selenium"
```
...
- [ ] Tests Playwright : création, édition, suppression, statut - tous verts
